var Theme = {
    ...
    init:function( $ ) {
        ...
        this.initDatePicker($);
        this.mobileChecker($);
        this.mobileNavScripts($);
        this.initSearchText($);
        this.initPrintingScripts($);
        this.initGasolineAutoCalc($);
        this.initPlateSorting($);
        ...
        $('.layout-container').css('max-width', newwidth+'px');
    ...
    initGasolineAutoCalc: function($){
        if($('.odo-container').length > 0){
            $('.sales-container .acf-actions .acf-button.button').click(function(){
                $('div[data-name="number_of_liters"] input').unbind('change');
                $('div[data-name="number_of_liters"] input').unbind('keyup');

                setTimeout(function(){
                    $('div[data-name="number_of_liters"] input').on('keyup change',function(){
                        var price = parseFloat($('div[data-name="price_per_liter"] input').val());

                        if(price > 0){
                            var computed = ($(this).val().length > 0)?price * parseFloat($(this).val()):0;

                            $(this).parents('td.acf-fields').find('div[data-name="amount"] input').val(computed.toFixed(2))
                        }
                    });
                }, 500);
            });

            if($('div[data-name="number_of_liters"] input').length > 0){
                $('div[data-name="number_of_liters"] input').on('keyup change',function(){
                    var price = parseFloat($('div[data-name="price_per_liter"] input').val());
    ...
    initPlateSorting: function($){
        var pages = '.page-template-template-rentals, .page-template-template-gasoline';
        
        if($(pages).length > 0){
            var sortref = ['FHN-216','VX-9335','ABN-1902','7AT-157324','GXW-196','7AT-113974','A1U-880','070110','A4G-840','GAL-9279','A9W-872','7CE-1441','7AT-134638','CCD-375','GAL-2986','GAH-6844','GXP-558','GAU-1296','AXA-6449','GXJ-936','GXW-171','CCE-597','GXT-469','GAN-9069','GXV-722','GXW-552','GXS-682','GXV-651','GXV-690','ABN-6165','A4G-796','GAO-7809','GAO-7635'];

            var count = (sortref.length - 1);

            while(count >= 0){
                var table = $('.search-table tbody');
                table.find('td:contains("'+sortref[count]+'")').parent().prependTo(table);
    ...
    initPrintingScripts: function($){
        if($('.printing__button').length > 0){

            $('.printing__init').click(function(e){
                e.preventDefault();

                var inner = '';

                inner += '<h1>KR TRANS WHEELERS CAB, INC.</h1>';
                inner += '<div class="printing__subtitle">'+$(this).attr('data-title')+'</div>';
                inner += '<div class="printing__date">Date: <span>'+$(this).attr('data-date')+'</span></div>';
                inner += '<div class="printing__table"><table class="main-table">'+$('table.search-table').html()+'</table><div class="summary-print">'+$('.summary').html()+'</div></div>';
                inner += '<div class="print-button">Print Document</div>';

                $('.printing__inner').html(inner);

                if($(this).attr('data-mode') != '1'){
                    $('.main-table thead tr th:last-child').remove();
                    $('.main-table tbody tr td:last-child').remove();
                }
            ...
            $('.printing__close').click(function(){
                $('.printing').hide();
            });
        }
    },
    ...
    printDiv: function() {
        var elementId = 'print-canvas';
        var b = document.getElementById(elementId).innerHTML;

        window.frames["print_frame"].document.title = document.title;
        window.frames["print_frame"].document.body.innerHTML = Theme.printCss() + b;
        window.frames["print_frame"].window.focus();
        window.frames["print_frame"].window.print();
    },

    printCss: function(){
        return '<style>*{color:#000;font-family:Arial,Helvetica,sans-serif}h1{color:#000;font-weight:700;font-size:20px;padding:0;margin:0;text-align:center}.printing__subtitle{text-align:center;font-size:18px;text-decoration:underline}.printing__date{text-align:right;font-size:16px}.printing__date span{text-decoration:underline}.printing__table{margin-top:15px}.printing__table table{width:100%;border:solid 2px #000;border-collapse:collapse;margin:0 auto}.printing__table table thead tr{border:solid 2px #000}.printing__table table th{text-transform:uppercase;font-size:13px}.printing__table table td{font-size:11px}.printing__table table td:first-child{font-weight:700}.printing__table table td a{color:#000;cursor:unset;font-size:14px;text-decoration:none}.printing__table table td,.printing__table table th{border:solid 1px #000;padding:4px 7px;text-align:center}.printing__table .summary-print{max-width:350px;margin:7px 0 0}.printing__table .summary-print *{color:#000;font-size:14px;font-weight:700}.printing__table .summary-print table{border:none}.printing__table .summary-print table th,.printing__table .summary-print table td{border:none;padding:3px 7px;text-align:left}.print-button{display:none;}</style>';
    },

    initSearchText: function($){
        if($('.search-text').length > 0){
            var stable = $('.search-table tbody tr');
            ...
            $('.search-text').keyup(function(){
                ...
                if(text.length > 0){
                    console.log(text);
                    stable.hide();
                    stable.find('td:contains("'+text+'")').parent().show();
                }else{
                    stable.show();
                }
            });

            Theme.initSearchProdName($);
        }
    },

    initSearchProdName: function($){
        if($('#search-prod').length > 0){
            var v = $('#search-prod').val();
    ...
    mobileNavScripts: function($){
        if(Theme.isMobile()){
            $('.main-nav-header h1').click(function(){
                var but = $('.main-nav-header');
    ...
    initDatePickerFilter: function($){
        if($('.search-date-filter .date-filter .date-text').length > 0){
            $('.search-date-filter .date-filter .date-text').datepicker({
                todayBtn: "linked",
                clearBtn: true,
                autoclose: true,
                todayHighlight: true
            ...
            $('.search-date-filter .date-filter input.button').click(function(e){
                e.preventDefault();

                if($(".search-date-filter .date-filter .date-text").val().length > 0){
                    $('.search-date-filter .date-filter input[name="end"]').val($(".search-date-filter .date-filter .date-text").val());

                    $('.date-filter form').submit();
                }
            });

            $(".search-date-filter .date-filter .date-text").keyup(function(e){
                $(this).val('');
            }); 
        }
    },
    ...
    initDatePicker: function($){
        Theme.initDatePickerFilter($);

        if($('.form-container .input-daterange input.input-sm').length > 0){
            $('.form-container .input-daterange input.input-sm').datepicker({
                todayBtn: "linked",
                clearBtn: true,
                autoclose: true,
                todayHighlight: true
            ...
            $(".form-container .input-daterange input.input-sm").attr("placeholder", months[date.getMonth()] +' '+ date.getDate() +', '+ date.getFullYear());

            /**** datepicker validation */

            $('.datepick-btn').click(function(e){
                e.preventDefault();

                var start = new Date($('.input-daterange input[name="start"]').val());
                var end = new Date($('.input-daterange input[name="end"]').val());

                if(start > end){
                    alert('Start Date should not be ahead of the End Date. Kindly edit the range setup.');
                }else{
        ...
        if($('.form-container.sales').length > 0){
            $('.hasDatepicker').attr('placeholder', 'Select Purchase Date');
        }
    },
    ...
    mainNavScripts: function($){
        var nav = $('.main-nav-header');

        nav.find('#navigation > .menu-item a').click(function(){
            var p = $(this).parent();
            ...
            if(p.hasClass('open')){
                ...
                var items = p.find('.sub-menu li').length;
                var h = p.height();
    
                p.css('height', h + (55 * items) + 'px');
                p.addClass('open');
            }
        });

        nav.find('#navigation > .menu-item').each(function(){
            if($(this).hasClass('current-menu-parent')){
                var items = $(this).find('.sub-menu li').length;
                var h = $(this).height();
    
                $(this).css('height', h + (55 * items) + 'px');
                $(this).addClass('open');
            }
        });
    },
